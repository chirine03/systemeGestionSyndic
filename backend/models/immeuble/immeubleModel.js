...
export const addImmeuble = async (immeuble) => {
    const { raison_sociale ,telephone ,adresse ,nbr_etage ,bloc ,description } = immeuble;
    const [result] = await connection.execute(
        'INSERT INTO immeuble ( raison_sociale ,telephone ,adresse ,nbr_etage ,bloc ,description ) VALUES (?, ?, ?, ?, ?, ?)',
        [ raison_sociale ,telephone ,adresse ,nbr_etage ,bloc ,description ]
    );
...
export const updateImmeuble = async (id_immeuble, immeuble) => {
    const { raison_sociale, telephone, adresse, nbr_etage, bloc, description } = immeuble;
    const [result] = await connection.execute(
        'UPDATE immeuble SET raison_sociale = ?, telephone = ?, adresse = ?, nbr_etage = ?, bloc = ?, description = ? WHERE id_immeuble = ?',
        [raison_sociale, telephone, adresse, nbr_etage, bloc, description, id_immeuble]
    );
    return result.affectedRows > 0;
};
